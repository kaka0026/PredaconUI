...
@Component({
  selector: 'app-dash-todaymakeable',
  templateUrl: './dash-todaymakeable.component.html',
  styleUrls: ['./dash-todaymakeable.component.css']
})
export class DashTodaymakeableComponent implements OnInit {
  ...
  LoadGridData() {
    ...
    this.spinner.show()
    this.DashboardServ.DashTodayMakeableFill(SaveObj).subscribe((FillRes) => {
  ...
  FillViewPara() {
    this.ViewParaMastServ.ViewParaFill({ FORMNAME: 'DashToDayMakable' }).subscribe((VPRes) => {
      try {
        if (VPRes.success == 1) {
