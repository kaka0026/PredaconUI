...
@Component({
  selector: 'app-dash-currolling',
  templateUrl: './dash-currolling.component.html',
  styleUrls: ['./dash-currolling.component.css']
})

export class DashCurrollingComponent implements OnInit {
  public columnDefs;
  public gridApi;
  ...
  FillViewPara() {
    this.ViewParaMastServ.ViewParaFill({ FORMNAME: 'DashCurRolling' }).subscribe((VPRes) => {
      try {
        if (VPRes.success == 1) {
  ...
  LoadGridData() {
    ...
    this.spinner.show()
    this.DashboardServ.DashCurrentRollingFill(SaveObj).subscribe((FillRes) => {
