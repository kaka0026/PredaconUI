...
@Component({
  selector: 'app-dash-prepending',
  templateUrl: './dash-prepending.component.html',
  styleUrls: ['./dash-prepending.component.css']
})
export class DashPrependingComponent implements OnInit {
  ...
  LoadGridData() {
    ...
    this.spinner.show()
    this.DashboardServ.DashPredictionPendingFill(SaveObj).subscribe((FillRes) => {
  ...
  FillViewPara() {
    this.ViewParaMastServ.ViewParaFill({ FORMNAME: 'DashPradicitonPending' }).subscribe((VPRes) => {
      try {
        if (VPRes.success == 1) {
