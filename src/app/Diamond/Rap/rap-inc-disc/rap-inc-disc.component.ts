...
export class RapIncDiscComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.itypes = this.decodedMast[7].map(item => {
      return { code: item.I_TYPE, name: item.I_TYPE };
    });
  ...
  IncFill() {
    this.IncMastServ.IncMastFill({ I_TYPE: this.IT_NAME, I_CODE: 0 }).subscribe(IncRes => {
      try {
        if (IncRes.success == true) {
          this.incs = IncRes.data.map(item => {
            return { code: item.I_CODE, name: item.I_NAME };
          });
          this.filteredIncs = this.incCtrl.valueChanges
            .pipe(
              startWith(''),
              map(grp => grp ? this.filterIncs(grp) : this.incs)
            );
        } else {
          this.spinner.hide();
          this.toastr.warning('Something gone wrong while get inclusion');
        }
      } catch (error) {
        this.spinner.hide();
        this.toastr.error(error);
  ...
  getIncName() {
    if (this.I_CODE) {
      if (this.incs.filter(option => option.code == this.I_CODE).length != 0) {
        this.I_NAME = this.incs.filter(option => option.code == this.I_CODE)[0].name
      } else {
        this.I_NAME = ''
      }
    } else {
  ...
  EXPORT() {
    this.RapIncDiscServ.RapIncDiscExport({ RAPTYPE: this.RAPTYPE, RTYPE: this.RTYPE, I_TYPE: this.IT_NAME }).subscribe(rapres => {
      try {
        if (rapres.success == true) {
          this.downloadFile(rapres.data, "INCDISC");
        } else {
          this.spinner.hide();
          this.toastr.warning('Something gone wrong while get shape');
        }
  ...
  IMPORT($event: any) {
    ...
    reader.onload = () => {
      ...
      if (newRecords[0].length != 0) {
        ...
        this.RapIncDiscServ.RapIncDiscImport(RapSaveArr).subscribe((SaveRes) => {
          this.spinner.hide()
          try {
            if (SaveRes.success == true) {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Rap not change with this criteria' + JSON.stringify(SaveRes.data)
              });
            }
          } catch (error) {
            this.toastr.error(error)
  ...
  downloadFile(data, filename = 'data') {
    let hadder = ['RAPTYPE', 'RTYPE', 'SZ_CODE', 'S_CODE', 'C_CODE', 'FL_CODE', 'CT_CODE', 'Q1', 'Q2', 'Q3', 'Q4', 'Q5', 'Q6', 'Q7', 'Q8', 'Q9', 'Q10', 'Q11', 'Q12', 'Q13', 'Q14', 'Q16', 'Q17']
