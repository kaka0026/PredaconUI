...
import { RapParamDiscService } from 'src/app/Service/Rap/rap-param-disc.service';
...
import { IncTypeMastService } from 'src/app/Service/Master/inc-type-mast.service';
...
@Component({
  selector: 'app-rap-param-disc',
  templateUrl: './rap-param-disc.component.html',
  styleUrls: ['./rap-param-disc.component.css']
})
export class RapParamDiscComponent implements OnInit {
  ...
  filteredParams: Observable<any[]>;
  params: Emp[] = [
    { code: '0', name: 'DIAMETER' },
    { code: '1', name: 'DEPTH' },
    { code: '2', name: 'TABLE' },
    { code: '3', name: 'RATIO' }];

  paramcCtrl: FormControl;
  filteredParamCs: Observable<any[]>;
  filteredMeters: ParamC[] = [];
  ...
  constructor(
    ...
    private RapParamDiscServ: RapParamDiscService,
    ...
    this.paramcCtrl = new FormControl();
    this.filterarr('filteredParams', 'paramCtrl', 'params');
    ...
    this.RapMastServ.RapTypeFill({ TYPE: 'RTYPE', TABNAME: '' }).subscribe(
      RRes => {
        try {
  ...
  DiameterFill() {
    if (this.S_CODE == '' || this.S_CODE == null) {
      this.toastr.warning('Please enter shape.');
      return;
    }

    if (this.P_NAME == '' || this.P_NAME == null) {
      this.toastr.warning('Please enter param.');
      return;
    }

    // console.log("PARAM NAME: ", this.P_NAME);
    // console.log("SHAPE CODE: ", this.S_CODE);

    this.filteredMeters = this.decodedMast[40]
      .filter(
        item => item.PARAM_NAME == this.P_NAME && item.S_CODE == this.S_CODE
      )
      .map(map_item => {
        return {
          code: map_item.PARAM_CODE.toString(),
          name: map_item.PARAM_CODE.toString(),
          from: map_item.FMETER,
          to: map_item.TMETER
        };
      });
    // console.log(this.filteredMeters);

    this.filterarr('filteredParamCs', 'paramcCtrl', 'filteredMeters');
  }

  getMeter() {
    if (this.P_CODE) {
      if (this.filteredMeters.filter(option => option.code.toLocaleLowerCase().includes(this.P_CODE.toLowerCase())).length != 0) {
        this.F_METER = this.filteredMeters.filter(option => option.code.toLocaleLowerCase().includes(this.P_CODE.toLowerCase()))[0].from
        this.T_METER = this.filteredMeters.filter(option => option.code.toLocaleLowerCase().includes(this.P_CODE.toLowerCase()))[0].to
      } else {
        this.F_METER = ''
        this.T_METER = ''
      }
  ...
  getCutName() {
    if (this.CT_CODE) {
      if (this.cuts.filter(option => option.code.toLocaleLowerCase().includes(this.CT_CODE.toLowerCase())).length != 0) {
        this.CT_NAME = this.cuts.filter(option => option.code.toLocaleLowerCase().includes(this.CT_CODE.toLowerCase()))[0].name
      } else {
        this.CT_NAME = ''
      }
    } else {
  ...
  LoadGridData() {
    this.spinner.show();
    this.RapParamDiscServ.RapParamDiscFill({
  ...
  Cellvaluechange(e: any) {
    ...
    if (e.newValue != 0 || e.newValue != null) {
      this.RapParamDiscServ.RapParamDiscSave({
        RAPTYPE: this.RAPTYPE,
        RTYPE: this.RTYPE,
        SZ_CODE: this.SZ_CODE,
        S_CODE: this.S_CODE,
  ...
  EXPORT() {
    this.RapParamDiscServ.RapParamDiscExport({
      RAPTYPE: this.RAPTYPE,
      RTYPE: this.RTYPE,
      PARAM_NAME: this.P_NAME
    }).subscribe(rapres => {
      try {
        if (rapres.success == true) {
          this.downloadFile(rapres.data, 'PARAMDISC');
        } else {
          this.spinner.hide();
          this.toastr.warning('Something gone wrong while get shape');
        }
      ...
      if (newRecords[0].length != 0) {
        ...
        for (let i = 0; i < newRecords[0].length; i++) {
          let SaveObj = {
            ...
            FL_CODE: newRecords[0][i].FL_CODE,
        ...
        this.RapParamDiscServ.RapParamDiscImport(RapSaveArr).subscribe(
          SaveRes => {
            this.spinner.hide();
            try {
              if (SaveRes.success == true) {
                ...
                Swal.fire({
                  ...
                  title: 'Oops...',
                  text:
                    'Rap not change with this criteria' +
