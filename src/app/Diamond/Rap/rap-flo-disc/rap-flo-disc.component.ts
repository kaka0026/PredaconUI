...
export class RapFloDiscComponent implements OnInit {
  ...
  EXPORT() {
    this.RapFloDiscServ.RapFloDiscExport({ RAPTYPE: this.RAPTYPE, RTYPE: this.RTYPE }).subscribe(rapres => {
      try {
        if (rapres.success == true) {
          this.downloadFile(rapres.data, "FLODISC");
        } else {
          this.spinner.hide();
  ...
  IMPORT($event: any) {
    ...
    reader.onload = () => {
      ...
      if (newRecords[0].length != 0) {
        ...
        this.RapFloDiscServ.RapFloDiscImport(RapSaveArr).subscribe((SaveRes) => {
          this.spinner.hide()
          try {
