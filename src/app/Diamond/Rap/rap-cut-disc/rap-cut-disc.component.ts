...
export class RapCutDiscComponent implements OnInit {
  ...
  getPrpName() {
    ...
    if (this.PRP_CODE) {
      if (this.Prps.filter(option => option.code == this.PRP_CODE).length != 0) {
        this.PRP_NAME = this.Prps.filter(option => option.code == this.PRP_CODE)[0].name
      } else {
        this.PRP_NAME = ''
      }
    } else {
  ...
  LoadGridData() {
    ...
    this.RapCutDiscServ.RapCutDiscFill({
      RAPTYPE: this.RAPTYPE, STYPE: this.PRPTYPE, RTYPE: this.RTYPE,
      SZ_CODE: this.SZ_CODE, S_CODE: this.S_CODE,
      C_CODE: 0, PRP: this.PRP_CODE, FL_CODE: this.FL_CODE
    }).subscribe(RapRes => {
      try {
        if (RapRes.success == true) {
          this.spinner.hide();
  ...
  PRPCHANGE() {
    ...
    if (this.PRPTYPE == "PRP") {
      this.Prps = this.decodedMast[9].map(item => {
        return { code: item.PR_CNAME, name: item.PR_NAME };
      });
    } else {
      this.Prps = this.decodedMast[5].map(item => {
  ...
  EXPORT() {
    this.RapCutDiscServ.RapCutdExport({ RAPTYPE: this.RAPTYPE, RTYPE: this.RTYPE, STYPE: this.PRPTYPE }).subscribe(rapres => {
      try {
        if (rapres.success == true) {
          this.downloadFile(rapres.data, "CUTDISC");
        } else {
          this.spinner.hide();
          this.toastr.warning('Something gone wrong while get shape');
        }
  ...
  downloadFile(data, filename = 'data') {
    let hadder = ['RAPTYPE', 'STYPE', 'RTYPE', 'SZ_CODE', 'S_CODE', 'C_CODE', 'PRP', 'Q1', 'Q2', 'Q3', 'Q4', 'Q5', 'Q6', 'Q7', 'Q8', 'Q9', 'Q10', 'Q11', 'Q12', 'Q13', 'Q14', 'Q16', 'Q17']
  ...
  IMPORT($event: any) {
    ...
    reader.onload = () => {
      ...
      if (newRecords[0].length != 0) {
        ...
        this.RapCutDiscServ.RapCutdImport(RapSaveArr).subscribe((SaveRes) => {
          this.spinner.hide()
          try {
